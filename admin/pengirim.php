...
$data_provinsi = getData("SELECT 'KOTA/KAB'.NAMA_KOTA, PROVINSI.ID_PROVINSI, PROVINSI.NAMA_PROVINSI FROM PENGIRIM JOIN 'KOTA/KAB' ON PENGIRIM.ID_KOTA = 'KOTA/KAB'.ID_KOTA JOIN PROVINSI ON 'KOTA/KAB'.ID_PROVINSI = PROVINSI.ID_PROVINSI GROUP BY PROVINSI.ID_PROVINSI");
$data_idProv = [];
$data_namaProv = [];
...
if (isset($_GET['filterby']) and $_GET['filterby'] != "") {
	...
	$sortby = $_GET['sortby'];
	$sortto = $_GET['sortto'];
	$data = getData("SELECT PENGIRIM.ID_PENGIRIM, PENGIRIM.NAMA_PENGIRIM, PENGIRIM.NOTELP_CP, PENGIRIM.NAMA_CP, PENGIRIM.EMAIL_PENGIRIM, PENGIRIM.ALAMAT, 'KOTA/KAB'.NAMA_KOTA, PROVINSI.ID_PROVINSI, PROVINSI.NAMA_PROVINSI FROM PENGIRIM JOIN 'KOTA/KAB' ON PENGIRIM.ID_KOTA = 'KOTA/KAB'.ID_KOTA JOIN PROVINSI ON 'KOTA/KAB'.ID_PROVINSI = PROVINSI.ID_PROVINSI WHERE PROVINSI.ID_PROVINSI = '$filterby' ORDER BY $sortby $sortto");
} elseif (isset($_GET['sortby'])) {
	$sortby = $_GET['sortby'];
	$sortto = $_GET['sortto'];
	$data = getData("SELECT PENGIRIM.ID_PENGIRIM, PENGIRIM.NAMA_PENGIRIM, PENGIRIM.NOTELP_CP, PENGIRIM.NAMA_CP, PENGIRIM.EMAIL_PENGIRIM, PENGIRIM.ALAMAT, 'KOTA/KAB'.NAMA_KOTA, PROVINSI.ID_PROVINSI, PROVINSI.NAMA_PROVINSI FROM PENGIRIM JOIN 'KOTA/KAB' ON PENGIRIM.ID_KOTA = 'KOTA/KAB'.ID_KOTA JOIN PROVINSI ON 'KOTA/KAB'.ID_PROVINSI = PROVINSI.ID_PROVINSI ORDER BY $sortby $sortto");
} else {
	$data = getData("SELECT PENGIRIM.ID_PENGIRIM, PENGIRIM.NAMA_PENGIRIM, PENGIRIM.NOTELP_CP, PENGIRIM.NAMA_CP, PENGIRIM.EMAIL_PENGIRIM, PENGIRIM.ALAMAT, 'KOTA/KAB'.NAMA_KOTA, PROVINSI.ID_PROVINSI, PROVINSI.NAMA_PROVINSI FROM PENGIRIM JOIN 'KOTA/KAB' ON PENGIRIM.ID_KOTA = 'KOTA/KAB'.ID_KOTA JOIN PROVINSI ON 'KOTA/KAB'.ID_PROVINSI = PROVINSI.ID_PROVINSI");
}
?>
<!DOCTYPE html>
<html>
	...
	<div class="container-fluid">
	  <div class="row">
		...
		<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 pt-5">
			<h1 class="h2 my-5">Daftar Pengirim</h1>
			<div class="table-responsive mx-5">
					<table class="table">
						<tr>
							<td>
								<a href="tambahPengirim.php"><button class="btn btn-warning ">
									<img src="../assets/img/add-icon.svg" width="25px" height="25px" class="me-2"> Tambah Pengirim</button>
								</a>
							</td>
						</tr>
						...
						<tr>
							<td>
								...
								<select name="sortby" id="sortby" class="form-select">
									<option value="PENGIRIM.NAMA_PENGIRIM">Nama</option>
			...
			<div class="table-responsive mx-5 mt-3">
				...
				<thead>
					<tr>
						...
						<th scope="col" class="table-dark">CP</th>
				...
				<tbody>
					...
					<tr>
						...
						<td><?= $row['NAMA_CP']."( ".$row['NOTELP_CP']." )" ?></td>
						<td>
							<a href="editPengirim.php?id_pengirim=<?= $row['ID_PENGIRIM'] ?>" title="Sunting Data"><img alt="Sunting Data" src="../assets/img/edit3-icon.svg" width="25px" height="25px"></img></a>
							<a href="deletePengirim.php?id_pengirim=<?= $row['ID_PENGIRIM'] ?>" onClick="return confirm('Hapus Data Pengirim ini?')" title="Hapus Data"><img alt="Hapus Data" src="../assets/img/delete2-icon.svg" width="25px" height="25px" class="ms-2"></img></a>
						</td>
					</tr>
					<?php $i++ ?>
					<?php endforeach ?>
