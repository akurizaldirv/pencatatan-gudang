...
$data = getData("SELECT * FROM KATEGORI WHERE ID_KATEGORI = '$id_kategori'")[0];
if (isset($_POST['submit'])) {
	if (editKategori($_POST)>0) {
		echo "<script>alert('Berhasil Menyunting Kategori')</script>";
	} else {
		echo "<script>alert('Gagal Menyunting Kategori')</script>";
	}
		...
		<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 py-5">
			...
			<div class="container w-75 py-3">
				<form method="POST" action="">
					...
					<div class="mb-3">
						<label for="nama" name="nama" class="form-label">Nama Kategori</label>
						<input type="text" name="nama" class="form-control" id="nama" value="<?php echo $data['NAMA_KATEGORI'] ?>" required>
					</div>
					<div class="mb-3">
						<button type="submit" class="btn btn-primary" name="submit">Sunting</button>
						<a href="kategori.php"><button type="button" class="btn btn-danger" name="cancel">Batal</button></a>
					</div>
				</form>
			</div>
			</div>
